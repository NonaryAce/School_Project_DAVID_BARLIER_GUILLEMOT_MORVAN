oxford = function(nchain, data, init, prop.sd){
  ...
  ## Mettre des commentaires sur ce que fait la fonction 
  ...
  for (iter in 1:nchain){
    ...
    for (j in 1:120){
      ...
      bottom = ((-1/2)*(10^-(6))*mu[j]^2) + r0[j]*log(p0_current) +
        (n0[j]-r0[j])*log(1-p0_current)
...
# Choix de prop.sd
prop.sd = matrix(c(0.062,0.016,0.003,1.2,0.8), ncol=5)
...
## Pour beta_1
plot(out$chain[,"Beta_1"], type='l')
plot(density(out$chain[,"Beta_1"]), type='l')
mean(out$chain[,"Beta_1"])
sd(out$chain[,"Beta_1"])
...
## Pour beta_2
plot(out$chain[,"Beta_2"], type='l')
plot(density(out$chain[,"Beta_2"]), type='l')
mean(out$chain[,"Beta_2"])
sd(out$chain[,"Beta_2"])
...
## Pour sigma
plot(out$chain[,"Sigma"], type='l')
plot(density(out$chain[,"Sigma"]), type='l')
mean(out$chain[,"Sigma"])
sd(out$chain[,"Sigma"])
